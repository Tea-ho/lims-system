...
```
src/main/java/com/lims/lims_system/
...
### 2. 데이터베이스 설정
```sql
CREATE DATABASE lims_system;
CREATE USER 'lims_system'@'localhost' IDENTIFIED BY 'rlaxogh123!';
GRANT ALL PRIVILEGES ON lims_system.* TO 'lims_system'@'localhost';
```
...
# 생성된 JAR 실행
java -jar build/libs/lims-system-0.0.1-SNAPSHOT.jar
```
...
### 로그 파일
로그는 다음 위치에 저장됩니다:
- 일반 로그: `logs/lims_system.log`
- 에러 로그: `logs/lims_system-error.log`
- SQL 로그: 콘솔 출력 (개발 환경)
